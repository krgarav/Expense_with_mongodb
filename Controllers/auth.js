    ...
    const postSignup = async () => {
        try {
            ...
            bcrypt.hash(password, 10, async (err, hash) => {
                ...
                const user = new User({ name, email, password: hash, totalExpense: 0, ispremiumuser: 0 });
                await user.save()
...
exports.updatePassword = async (req, res) => {
    ...
    try {
        bcrypt.hash(password, 10, async (err, hash) => {
            ...
            const forgotTable = await Forgotpassword.findOne({ _id: userId })
            const clientId = forgotTable.userId;
            forgotTable.isActive = false;
            await forgotTable.save();
