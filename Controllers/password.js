...
exports.sendMail = async (req, res) => {
    ...
    try {
        ...
        const newForgotRequest = new Forgotpasswordrequests({userId: user._id, isActive: true});
        await newForgotRequest.save();
        const requestUser = await Forgotpasswordrequests.findOne({ userId: user._id, isActive: true });
        const forgotRequest = requestUser._id;
...
const deactivateForgotPasswordRequest = async (requestId) => {
    try {
        const requestUser = await Forgotpasswordrequests.findOne({ id: requestId, isActive: true  });
